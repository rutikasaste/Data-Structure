...
Each move consists of taking the upper disk from one of the stacks and placing it on top of another stack i.e. a disk can only be moved 
if it is the uppermost disk on a stack.
No disk may be placed on top of a smaller disk.*/

#include<stdio.h>

void Tow(int N ,char from_rod , char via_rod , char to_rod){
    if(N>0){
        Tow(N-1 ,from_rod ,to_rod,via_rod);
        printf("Move disk %d From %c -> %c\n" ,N,from_rod ,to_rod);
        Tow(N-1 ,via_rod ,from_rod ,to_rod);
    }
